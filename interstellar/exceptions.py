...
class CancelledError(InterstellarError):
    """
    The operation was cancelled, typically by the caller.
    """
...
class UnknownError(InterstellarError):
    """
...
class DeadlineExceededError(InterstellarError):
    """
    The deadline expired before the operation could complete.
    For operations that change the state of the system, this error may
    be returned even if the operation has completed successfully.
...
class NotFoundError(InterstellarError):
    """
...
class PermissionDeniedError(InterstellarError):
    ...
    does not imply the request is valid or the requested entity
...
class UnauthenticatedError(InterstellarError):
    """
...
class ResourceExhaustedError(InterstellarError):
    """
...
class FailedPreconditionError(InterstellarError):
    """
    The operation was rejected because the system is not in a state required
    for the operation's execution. For example, the directory to be deleted
    is non-empty, an rmdir operation is applied to a non-directory,
    etc. Service implementors can use the following guidelines to
    decide between FAILED_PRECONDITION, ABORTED, and UNAVAILABLE:
...
class AbortedError(InterstellarError):
    """
...
class OutOfRangeError(InterstellarError):
    """
    The operation was attempted past the valid range.
    E.g., seeking or reading past end-of-file.
    Unlike INVALID_ARGUMENT, this error indicates a problem that may
    be fixed if the system state changes. For example,
    a 32-bit file system will generate INVALID_ARGUMENT if asked to
    read at an offset that is not in the range [0,2^32-1],
    but it will generate OUT_OF_RANGE if asked to read from an offset
    past the current file size. There is a fair bit of overlap
    between FAILED_PRECONDITION and OUT_OF_RANGE. We recommend
    using OUT_OF_RANGE (the more specific error) when it applies
    so that callers who are iterating through a space can easily
...
class UnimplementedError(InterstellarError):
    """
...
class InternalError(InterstellarError):
    """
...
class UnavailableError(InterstellarError):
    """
    The service is currently unavailable. This is most likely a
    transient condition, which can be corrected by retrying
...
class DataLossError(InterstellarError):
    """
    Unrecoverable data loss or corruption.
    """
